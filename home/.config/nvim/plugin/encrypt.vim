...
function! s:decrypt_read () abort
  ...
  if filereadable (bufname ())
    ...
    silent execute '%!openssl aes-256-cbc -d -iter 101011 -k' shellescape (password) '-in' shellescape (bufname ())
    setlocal nomodified
...
function! s:encrypt_write () abort
  ...
  if password ==# password2
    let res = execute (printf ('write !openssl aes-256-cbc -e -iter 101011 -k %s -out %s', shellescape (password), shellescape (bufname ())))
    if v:shell_error
      call s:echo_err (res)
    else
      setlocal nomodified
